...
case class Cache(ttl: Int = 600) {
  ...
  def addItem(response: String): Unit = addToFile(name = json.getID(response).toString, content = response)

  def addUser(response: String): Unit = addToFile(name = json.getuserID(response), content = response)
  ...
  def getCache(name: String): String = getFile(name)

  def exists(name: String): Boolean = Files.exists(Paths.get(location + name + ".json"))
